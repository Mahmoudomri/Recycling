...
const Modifiersouscategories = ({ navigation }) => {
  ...
  const renderItem = ({ item }) => (
    <TouchableOpacity onPress={() => navigation.navigate('Modifff', { id: item.id })}>
      <View style={styles.item}>
        <Text style={{ flex: 1, fontSize: 16, fontWeight: 'bold',backgroundColor: '#8ED332', color: 'white', marginRight:60,paddingVertical: 10,borderRadius: 5,opacity: 0.9,}}>  {item.titledeux}</Text>
...
export default Modifiersouscategories;
